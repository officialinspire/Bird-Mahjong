...
describe("settings and bests with bad storage", () => {
  test("settings: missing, corrupt or partial data falls back field by field", () => {
    ...
    storage.setItem(SETTINGS_KEY, JSON.stringify({ sfxVolume: 7, sfx: 1 }));
    assert.deepEqual(loadSettings(storage), { ...DEFAULT_SETTINGS, sfxVolume: 1 }, "volume clamped, bad flag ignored");
    storage.setItem(SETTINGS_KEY, JSON.stringify({ musicVolume: "loud" }));
    assert.equal(loadSettings(storage).musicVolume, DEFAULT_SETTINGS.musicVolume);
  ...
  test("settings round-trip", () => {
    const { storage } = openStorage(memoryStorage());
    const custom = { motion: "reduce", backgroundBirds: false, tileLabels: true, streakBonus: false, music: false, musicVolume: 0.1, sfx: false, sfxVolume: 0.25 };
